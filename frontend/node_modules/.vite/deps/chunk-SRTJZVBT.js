...
function tokenizeCodeFenced(effects, ok2, nok) {
  ...
  function openAfter(code) {
    ...
    return self.interrupt ? ok2(code) : contentStart(code);
  }
...
function tokenizeIndentedContent(effects, ok2, nok) {
  ...
  function afterPrefix(code) {
    const tail = self.events[self.events.length - 1];
    return tail && tail[1].type === types.linePrefix && tail[2].sliceSerialize(tail[1], true).length >= constants.tabSize ? ok2(code) : markdownLineEnding(code) ? start(code) : nok(code);
  }
}
...
function factoryLabel(effects, ok2, nok, type, markerType, stringType) {
  ...
  function atBreak(code) {
    if (code === codes.eof || code === codes.leftSquareBracket || code === codes.rightSquareBracket && !data || /* To do: remove in the future once we’ve switched from
...
function tokenizeHtmlFlow(effects, ok2, nok) {
  ...
  function htmlLineEnd(effects2, ok3, nok2) {
    ...
    function lineStart(code) {
      return self.parser.lazy[self.now().line] ? nok2(code) : ok3(code);
    }
  }
...
//# sourceMappingURL=chunk-SRTJZVBT.js.map
